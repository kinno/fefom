<template>
  <v-card>
    ...
    >
                ...
                <v-container>
                  <v-row>
                    ...
                    <v-col
                      cols="12"
                      md="6"
                      v-if="editedItem.tipo_usuario.id == 1"
                    >
                      <v-select
                    ...
                    <v-col
                      cols="12"
                      md="6"
                      v-if="editedItem.tipo_usuario.id == 2"
                    >
                      <!-- <v-select
      ...
      <template v-slot:item.action="{ item }">
        ...
        <v-btn
          ...
          x-small
          height="30px"
          @click="newPassword(item)"
        >
          <v-icon color="green darken-2">mdi-shield-lock-outline</v-icon>
        </v-btn>
        <v-btn
          class="btnK pa-0"
          x-small
    ...
    <v-dialog v-model="password" max-width="300px">
      <v-card>
        <v-card-title>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="green darken-1" text @click="closePass">Cancelar</v-btn>
          <v-btn color="green darken-1" text @click="savePass">Guardar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-card>
...
export default {
  ...
  methods: {
          ...
          response.data.rows.forEach(element => {
            ...
            this.desserts.push({
              ...
              tipo_usuario: {
                ...
                descripcion:
                  element.tipo_usuario == 1
                    ? "Administrativo FEFOM"
                    : "Ayuntamiento"
              },
              tipo_rol: tipo_rol
            });
    ...
    save() {
      this.$http
        .post("/catalogos/registrar_usuario", {
          id:
          ...
          id_municipio:
            this.editedItem.municipio !== null
              ? this.editedItem.municipio.id_municipio
              : null,
          activo: 1,
          tipo_usuario: this.editedItem.tipo_usuario.id,
          tipo_rol:
            this.editedItem.tipo_rol !== null
              ? this.editedItem.tipo_rol.id
              : null,
